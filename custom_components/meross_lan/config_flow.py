    ...
    async def async_step_cloudkey(self, user_input=None):
        ...
        if user_input:
            ...
            try:
                ...
                await MerossApi.async_update_profile(self.hass, credentials)
                self._key = credentials.key
                self._cloud_key = credentials.key
                self._cloud_profile = credentials.userid
...
class ConfigFlow(MerossFlowHandlerMixin, config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_device(self, user_input=None):
        ...
        if user_input is not None:
            ...
            try:
                _discovery_info = await _http_discovery(
                    self.hass, self._host, self._key
                )
        ...
        try:
            ...
            if api is not None:
                ...
                if api.cloud_key is not None:
                    try:
                        _discovery_info = await _http_discovery(
                            self.hass, self._host, api.cloud_key
                        )
                        self._key = api.cloud_key
                        self._cloud_key = api.cloud_key
                    except MerossKeyError:
                        pass
                if (
                    (_discovery_info is None)
                    and (api.key is not None)
                    and (api.key != api.cloud_key)
                ):
                    try:
                        _discovery_info = await _http_discovery(
                            self.hass, self._host, api.key
                        )
                        self._key = api.key
                    except MerossKeyError:
                        pass
...
class OptionsFlowHandler(MerossFlowHandlerMixin, config_entries.OptionsFlow):
    ...
    def __init__(self, config_entry: config_entries.ConfigEntry):
        ...
        if config_entry.unique_id != DOMAIN:
            ...
            self._cloud_profile = data.get(CONF_CLOUD_PROFILE)
    ...
    async def async_step_device(self, user_input=None):
        ...
        errors = {}
        api = MerossApi.get(self.hass)
        device = api.devices.get(self._device_id)  # type: ignore
        if user_input is not None:
            self._host = user_input.get(CONF_HOST)
            self._key = user_input.get(CONF_KEY)
            self._protocol = user_input.get(CONF_PROTOCOL)
            self._cloud_profile = user_input.get(CONF_CLOUD_PROFILE)
            ...
            try:
                if self._host is not None:
                    ...
                    _descriptor = MerossDeviceDescriptor(_discovery_info[CONF_PAYLOAD])  # type: ignore
                    if self._device_id != _descriptor.uuid:
                        raise ConfigError(ERR_DEVICE_ID_MISMATCH)
                    if self._cloud_profile and _descriptor.userId != self._cloud_profile:
                        raise ConfigError(ERR_CLOUD_PROFILE_MISMATCH)
                data = dict(self._config_entry.data)
                if self._host is not None:
                    data[CONF_HOST] = self._host
                    data[CONF_PAYLOAD] = _discovery_info[CONF_PAYLOAD] # type: ignore
        ...
        config_schema[
            vol.Optional(CONF_KEY, description={DESCR: self._key})
        ] = str
        config_schema[
