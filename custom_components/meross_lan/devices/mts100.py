...
from ..calendar import (
    MLCalendar, CalendarEvent, CalendarEntityFeature,
...
class Mts100Schedule(MLCalendar):
    ...
    def _get_next_event_entry(self, event_entry: Mts100ScheduleEntry) -> Mts100ScheduleEntry | None:
        try:
            ...
            self.device.log(
                WARNING, 14400,
                "Mts100Schedule(%s) error parsing internal schedule: %s",
                self.name, str(error)
            )
            return None
    ...
    async def async_get_events(
        ...
        while event_entry is not None:
            ...
            if len(events) > 1000:
                self.device.log(
                    WARNING, 14400,
                    "Mts100Schedule(%s) returning too many calendar events: breaking the loop now",
                    self.name
                )
                break
    ...
    async def async_create_event(self, **kwargs):
        try:
            ...
            try:
                event_temperature = int(clamp(float(kwargs[EVENT_SUMMARY]), self.climate.min_temp, self.climate.max_temp) * 10)
            except Exception as error:
                raise Exception("Provide a valid °C temperature in the summary field") from error
            ...
            if event_end == event_day_end:
                ...
                if event_minutes_end_remainder:
                    event_minutes_end += self._scheduleunittime - event_minutes_end_remainder
            event_minutes_duration = event_minutes_end - event_minutes_start
            if event_minutes_duration < self._scheduleunittime:
                raise Exception(f"Minimum event duration is {self._scheduleunittime} minutes")
            ...
            for weekday in recurrencedays:
                ...
                for schedule_entry in weekday_schedule:
                    ...
                    if event_minutes_start < schedule_minutes_begin:
                        ...
                        weekday_schedule.insert(schedule_index_insert, [event_minutes_duration, event_temperature])
                        ...
                        while _event_minutes_duration:
                            ...
                            if schedule_entry_minutes_duration > _event_minutes_duration:
                                ...
                                schedule_entry[0] = schedule_entry_minutes_duration - _event_minutes_duration
                                break
                            ...
                            assert _event_minutes_duration >= schedule_entry_minutes_duration, "Something wrong in our schedule"
                        ...
                        if event_minutes_end < schedule_minutes_end:
                            ...
                            weekday_schedule.insert(schedule_index + 2, [schedule_minutes_end - event_minutes_end, schedule_entry[1]])
                            ...
                            _event_minutes_duration = event_minutes_duration - schedule_minutes_end - event_minutes_start
                            schedule_index += 2
                            while _event_minutes_duration > 0:
                                ...
                                weekday_schedule.pop(schedule_index)
                                _event_minutes_duration -= schedule_entry_minutes_duration
                            assert _event_minutes_duration == 0, "Something wrong in our schedule"
                        break
                    schedule_minutes_begin = schedule_minutes_end
                    schedule_index += 1
                ...
                while (len(weekday_schedule) > schedule_count):
                    ...
                    if schedule_index_insert > 1:
                        ...
                        continue
                    schedule_entries_after = len(weekday_schedule) - schedule_index_insert - 1
        ...
        if schedule := self.schedule:
            ...
            for weekday, weekday_schedule in schedule.items():
                ...
                for schedule_entry in weekday_schedule:
                    ...
                    while (schedule_entry_duration > schedule_entry_unittime)\
                        and (schedule_items_missing > 0):
                        payload_weekday_schedule.append([
                            schedule_entry_unittime,
                            schedule_entry[1]]
                        )
                        schedule_entry_duration -= schedule_entry_unittime
                        schedule_items_missing -= 1
                    payload_weekday_schedule.append([
                        schedule_entry_duration,
                        schedule_entry[1]]
                    )
